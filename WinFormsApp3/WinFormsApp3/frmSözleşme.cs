...
namespace WinFormsApp3
{
    public partial class frmSözleşme : Form
    {
        ...
        private void btnEkle_Click(object sender, EventArgs e)
        {
            ...
            Boş_Araçlar();
            Yenile();
            foreach (Control item in groupBox1.Controls) if (item is TextBox) _ = item.Text == "";
            foreach (Control item in groupBox1.Controls) if (item is TextBox) _ = item.Text == "";
            comboAraçlar.Text = "";
            Temizle();
            MessageBox.Show("şözleşme eklendi");
        }

        private void txtTcAra_TextChanged(object sender, EventArgs e)
        {
            if (txtTcAra.Text == "") foreach (Control item in groupBox1.Controls) if (item is TextBox) _ = item.Text == "";
        ...
        private void btnGüncelle_Click(object sender, EventArgs e)
        {
            ...
            komut2.Parameters.AddWithValue("@gun", int.Parse(txtGün.Text));
            komut2.Parameters.AddWithValue("@tutar ", int.Parse(txtTutar.Text));
            komut2.Parameters.AddWithValue("@ctarih", dateÇıkış.Value);
            komut2.Parameters.AddWithValue("@dtarih", dateDönüş.Value);
            arac.ekle_sil_guncelle(komut2, sorgu2);
            ...
            comboAraçlar.Text = "";
            Temizle();
            MessageBox.Show("şözleşme güncellendi.");
        }
        ...
        private void btnAraçTeslim_Click(object sender, EventArgs e)
        {
            if (int.Parse(txtEkstra.Text) >= 0 || int.Parse(txtEkstra.Text) < 0)
            {
                ...
                Yenile();
                foreach (Control item in groupBox1.Controls) if (item is TextBox) _ = item.Text == "";
                foreach (Control item in groupBox2.Controls) if (item is TextBox) _ = item.Text == "";
                comboAraçlar.Text = "";
                Temizle();
                txtEkstra.Text = "";
